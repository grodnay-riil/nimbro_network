...
{
	RCLCPP_INFO(this->get_logger(), "nimbro_topic_transport v2.0.0 — UDP sender");
	...
	RCLCPP_INFO(this->get_logger(), "Configuration: destination=%s:%d, topics=%zu, fec=%.1f, relay=%s",
		dest_host.c_str(), dest_port, topic_names.size(), m_fec,
		m_relayMode ? "true" : "false");
}
